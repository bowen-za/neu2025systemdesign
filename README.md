...
## 新增功能（qt-gui-version-111 分支补丁）

以下功能在 `qt-gui-version-111` 分支中新增，基于 `qt-gui-version` 分支进行优化开发。

### 1. 回退键（Back Button）

登录后，每个界面左上角增加了"<"回退键，点击可回到上一层级的目录页面。`/usr1` 等用户家目录为导航边界，到达边界时回退键自动隐藏，不可继续向上。

### 2. 目录路径导航栏（Path Navigation Bar）

登录后界面最上方新增路径显示输入框，实时显示当前所在目录的完整路径。点击输入框可手动输入目标路径，按下回车键或点击确定后即可跳转至指定目录。若输入的路径不存在或非法，将弹窗报错并拒绝跳转。该功能参考 Windows 文件资源管理器的地址栏设计。

### 3. 拖动文件图标调整显示顺序（Drag to Reorder Icons）

在同一目录下存在多个文件或文件夹时，可通过鼠标拖动图标来更改它们在界面上的排列顺序。拖动后的顺序信息会被持久化保存，刷新或重新进入该目录时顺序保持不变。

### 4. 修复目录打开行为不一致

修复了双击打开目录与右键打开目录结果不一致的问题。现在无论双击还是右键点击目录，都会在当前界面内直接进入目录，不再产生新窗口，保证了交互行为的一致性。

### 5. 注册用户界面（Registration）

登录界面新增了"注册"选项，点击后进入注册界面。注册界面包含以下功能：

- 输入用户名、密码和确认密码
- 密码和确认密码内容必须相同才能注册成功
- 任何字段空缺均无法注册，报错方式与登录界面一致
- 注册界面保留"返回登录"按键，可随时返回登录界面
- 新建用户与默认 8 个用户具有相同的权限级别
- 格式化系统时，新建用户会被删除，保留原有的 8 个默认用户
